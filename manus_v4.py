...
class Plan(BaseModel):
    ...
    def mark_step_completed(self, step_index: int, result: str):
...
class PlanningTool(BaseTool):
    ...
    parameters: Dict = {
        ...
        "properties": {
            "command": {
                "type": "string",
                "enum": ["create", "update", "get", "mark_step"],
                "description": "Planning command to execute"
            },
            "plan_data": {
                "type": "object",
                "description": "Plan data for create/update operations"
            },
            "step_index": {
                "type": "integer",
                "description": "Step index for mark_step operation"
            },
            "result": {
                "type": "string",
                "description": "Result for mark_step operation"
            }
        },
        "required": ["command"]
    }
    ...
    async def execute(self, command: str, **kwargs) -> ToolResult:
        try:
            if command == "create":
                return await self._create_plan(kwargs.get("plan_data", {}))
            elif command == "update":
                return await self._update_plan(kwargs.get("plan_data", {}))
            elif command == "get":
                return await self._get_plan()
            elif command == "mark_step":
                return await self._mark_step_completed(
                    kwargs.get("step_index"),
                    kwargs.get("result", "")
                )
            else:
                return ToolResult(error=f"Unknown command: {command}", success=False)
        except Exception as e:
            return ToolResult(error=str(e), success=False)
    
    async def _create_plan(self, plan_data: Dict) -> ToolResult:
        """Create a new plan"""
        ...
        plan = Plan(
            ...
            steps=[
                PlanStep(index=i, description=step_desc, executor=step.get("executor"))
                for i, step in enumerate(plan_data.get("steps", []))
                for step_desc in [step.get("description", step) if isinstance(step, dict) else step]
            ]
        )
        ...
        return ToolResult(
            output=f"Created plan '{plan.title}' with {len(plan.steps)} steps",
            success=True
        )
    
    async def _get_plan(self) -> ToolResult:
        """Get current active plan"""
        if not self.active_plan_id or self.active_plan_id not in self.plans:
            return ToolResult(output="No active plan", success=True)
        
        plan = self.plans[self.active_plan_id]
        plan_status = {
            "id": plan.id,
            "title": plan.title,
            "status": plan.status,
        ...
        return ToolResult(output=plan_status, success=True)
...
class PlanningAgent(ReActAgent):
    ...
    def __init__(self, **kwargs):
        ...
        self.system_prompt = """
        You are a Planning Agent that excels at breaking down complex tasks into manageable steps.
        
        When given a task:
        1. Analyze the requirements thoroughly
        2. Create a detailed plan using the planning tool
        3. Execute steps one by one
        4. Track progress and adapt as needed
        5. Ensure all steps are completed before finishing
        
        Always think step-by-step and be methodical in your approach.
        """
    
    async def run(self, user_input: str) -> str:
        """Enhanced run method with planning"""
        self.messages.append(SimpleMessage(role="user", content=user_input))
        
        # First, create a plan
        await self._create_initial_plan(user_input)
        
        # Execute the plan step by step
        while True:
            # Get current plan status
            plan_result = await self.planning_tool.execute("get")
            plan_data = plan_result.output
            
            if isinstance(plan_data, dict):
                # Find next step to execute
                next_step = next(
                    (step for step in plan_data["steps"] if step["status"] == "pending"),
                    None
                )
    ...
    async def _create_initial_plan(self, task: str):
        ...
        Create a detailed plan to accomplish this task. Break it down into specific, actionable steps.
        Consider what tools you have available: {[tool.name for tool in self.available_tools.tools]}
        
        Use the planning tool to create the plan.
        """
        
        self.messages.append(SimpleMessage(role="user", content=planning_prompt))
        ...
        # Execute any tool calls (should be planning tool)
    ...
    async def _execute_step(self, step: Dict) -> str:
        ...
        This is step {step['index'] + 1} of the current plan.
        Focus only on this specific step and provide clear results.
        """
        
        self.messages.append(SimpleMessage(role="user", content=step_prompt))
        
        # Use ReAct pattern for step execution
        step_result = await self.step()
